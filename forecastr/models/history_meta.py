...
def _history_mapper(local_mapper):
    ...
    if not super_mapper or local_mapper.local_table is not super_mapper.local_table:
        ...
        if super_mapper:
            ...
            # @todo: default=func.current_timestamp()
            cols.append(Column('changed_at', DateTime, default=datetime.datetime.now))
        else:
            cols.append(Column('version', Integer, primary_key=True))
            # @todo: default=func.current_timestamp()
            cols.append(Column('changed_at', DateTime, default=datetime.datetime.now))
...
def create_version(obj, session, deleted = False):
    ...
    for om, hm in zip(obj_mapper.iterate_to_root(), history_mapper.iterate_to_root()):
        ...
        for hist_col in hm.local_table.c:
            if hist_col.key == 'version' or hist_col.key == 'changed_at':
                continue
